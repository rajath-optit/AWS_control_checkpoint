```
./mytestscrip2.sh 34567dfghjtyujUI tdcvbnmiuytrfvbnmkiuytfvbnkuyt67uij4567CE1 ap-south-1 pp-sp-img name-network my_custom_container
 ```
...
# Debug: Print the provided AWS credentials (for debugging only, remove in production)
...
initialize_and_install_modules() {
    ...
    sudo docker exec -it "$container_name" /bin/bash -c '
        ...
        yes | powerpipe mod install github.com/turbot/steampipe-mod-aws-compliance
    '
    # Run checks with environment variables
    sudo docker exec -it "$container_name" /bin/bash -c '
    ...
    '
    if [ $? -ne 0 ]; then
        echo "Error: Steampipe could not validate AWS connection. Please check your AWS credentials or permissions."
        exit 1
    fi
    echo "Modules initialized and installed successfully."
}
...
start_services() {
    ...
    '
    # Output Steampipe and Powerpipe logs for troubleshooting
    sudo docker exec -it "$container_name" /bin/bash -c 'tail -f /home/powerpipe/steampipe.log' &
    sudo docker exec -it "$container_name" /bin/bash -c 'tail -f /home/powerpipe/powerpipe.log' &
}
